...
$read_file = 'app/' . $term . '_' . $region . '.csv';

$f = fopen($read_file, 'r');

$data_t = [];

$dl_url = "./app/" . $term . '_' . $region . ".csv/";

while(($line = fgetcsv($f)) !== false) {
    array_push($data_t, $line);
}

$cities = array_unique(array_column($data_t, 16));
...
<div class="mt-6 px-4 sm:px-6 lg:px-8">
  <div class="sm:flex sm:items-center">
    <div class="sm:flex-auto">
      ...
      <p class="mt-2 text-sm text-gray-700">Lists of shops searched in 食べログ by term "<?=$term?>"</p>
      <p class="text-sm text-gray-60 mt-3">Update</p>
      
      <p class="text-xs text-gray-500 ml-3">10/4(火)：千葉の店舗を追加(パティスリー、洋菓子で検索)</p>
      <p class="text-xs text-gray-500 ml-3">9/30(金)：東京23区の店舗を追加</p>
      <!-- <p class="text-xs text-gray-500 ml-3">8/25(木)：csvのダウンロードが正常に機能するようになりました</p> -->
      
    </div>
    <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
      <a href="./cities.php?region=tokyo&term=yogashi">
        <button type="button" class="inline-flex items-center justify-center rounded-md border border-transparent bg-teal-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 sm:w-auto">東京_洋菓子</button>
      </a>
      <a href="./cities.php?region=chiba&term=patissery">
        <button type="button" class="inline-flex items-center justify-center rounded-md border border-transparent bg-teal-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 sm:w-auto">千葉_パティスリー</button>
      </a>
      <a href="./cities.php?region=chiba&term=yogashi">
        <button type="button" class="inline-flex items-center justify-center rounded-md border border-transparent bg-teal-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 sm:w-auto">千葉_洋菓子</button>
      </a>
      <?= '<a href="' . $dl_url . '" >' ?>
